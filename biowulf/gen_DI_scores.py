...
method_map = {0:'ER_clean',1:'ER_INIT',2: 'ER', 3:'MF',4:'PLM'}
...
def get_score(pfam_id, data_path = '/data/cresswellclayec/hoangd2_data/Pfam-A.full',preprocess_path = '/data/cresswellclayec/DCA_ER/biowulf/pfam_ecc/'):
	...
	print('\n#-------- ER ---------------------- MF --------------------------- PLM ------------------------ ER INIT ------------ ER clean----#')
	for i,pair in enumerate(DI_er[:25]):
		print(pair ,'  ',DI_mf[i],'  ',DI_plm[i], '  ',DI_er_coup[i], DI_er_clean[i])	
	print('#----------------------------------------------------------------------------------------------------------------------------------#\n')
	#--------------------------------------------------------------------------------#
	...
	#---------------------------------------------------------------------------------------------------------------------#
	contact_instance_labels = ['er_clean_visualizer','er_coup_visualizer','erdca_visualizer', 'mfdca_visualizer', 'plmdca_visualizer']
	contact_instances = [er_clean_visualizer, er_coup_visualizer,erdca_visualizer, mfdca_visualizer, plmdca_visualizer]
	scores = []
	...
	#---------------------------------------------------------------------------------------------------------------------#            
	DIs = [DI_er_clean, DI_er_coup, DI_er, DI_mf, DI_plm] # SHOULD HAVE SAME ORDER AS contact_instances!!!!
	AUCs = []
	...
	f.write('%s %f %f %f %f %f %d' % (pfam_id , AUCs[0], AUCs[1], AUCs[2], AUCs[3], AUCs[3], dca_num_seqs) )    
	f.close()
