...
def compute_single_site_freqs(alignment_data=None,
        num_site_states = None, unique_vals = None, seqs_weight=None):
    ...
    ----------
        ...
        num_site_states : int
            ...
            proteins.
        ***--> 
        unique_vals =  : list of int arrays
            Array of unique integer values for the interger states a sequence site has 
            including a gap state. Total values are 5 for RNAs and 21 for
            proteins.
    ...
    for i in range(seqs_len):
        ...
        state_counts = []
        for a in unique_vals[i]:			#we use varying site states (unique vals in col)
            freq_ia = np.sum((column_i==a)*seqs_weight)
            state_counts.append( freq_ia/m_eff)
        single_site_freqs.append(state_counts)
    return single_site_freqs


#@jit(nopython=True)
def get_reg_single_site_freqs(single_site_freqs = None, seqs_len = None, num_site_states = None,
    ...
    for i in range(seqs_len):
        ...
        #theta_by_q = np.float64(pseudocount)/np.float64(num_site_states)
        if i == 0:
            print('mx[0]: ',mx[0],' single_site_freqs[0]: ',reg_single_site_freqs[0])
        for a in range(mx[i]-1):
...
#@jit(nopython=True)
def compute_pair_site_freqs_serial(alignment_data=None, num_site_states = None,mx=None,
        seqs_weight=None):
    ...
    #    dtype = np.float64)
    pair_site_freqs = []		# accounting for variable num_site_states by position
    pair_counter = 0
...
#@jit(nopython=True, parallel=True)
def compute_pair_site_freqs(alignment_data=None, num_site_states=None, mx=None, seqs_weight=None):
    """Computes pair site frequencies for an alignmnet data.
    ...
    m_eff = np.sum(seqs_weight)
    #pair_site_freqs = np.zeros(
    pair_site_freqs = np.empty(
        shape=(num_site_pairs, num_site_states - 1, num_site_states - 1),
        dtype = np.float64
    )
    pair_site_freqs[:] = np.NaN # set to impossible count (to be removed after loop)
    ...
    for i in range(seqs_len - 1):
        ...
        for j in range(i+1, seqs_len):
            ...
            for a in range(1, i_site_states):
                count_ai = column_i==a
                for b in range(1, j_site_states):
                    count_bj = column_j==b
                    count_ai_bj = count_ai * count_bj
                    freq_ia_jb = np.sum(count_ai_bj*seqs_weight)
                    pair_site_freqs[pair_site, a-1, b-1] += freq_ia_jb/m_eff

    pair_site_freqs = pair_site_freqs[~np.isnan(pair_site_freqs)]	
    if any(pair_site_freqs[np.isnan(pair_site_freqs)]):
        print('indexing error in single_site_freqs')

    return pair_site_freqs 

#@jit(nopython=True)
def get_reg_pair_site_freqs(pair_site_freqs = None, seqs_len = None,num_site_states=None,
        mx = None, pseudocount = None):
    ...
    for i in range(seqs_len - 1):
        for j in range(i + 1, seqs_len):
            for a in range(mx[i]-1):
                for b in range(mx[j]-1):
                    #theta_by_qsqrd = pseudocount/float(mx[i] * mx[j])
...
#@jit(nopython=True)
def construct_corr_mat(reg_fi = None, reg_fij = None, seqs_len = None, mx = None,
    ...
    for i in range(seqs_len-1):
        if i==0:
            ...
            site_i = mx_cumsum[i-1]
        for j in range(i+1, seqs_len):
            #site_j = j * (num_site_states - 1)
            site_j = mx_cumsum[j-1]
            for a in range(mx[i] ):
                row = site_i + a
                for b in range(mx[j]):
                    col = site_j + b
