...
"""Python wrapper for Expectation Reflection direct coupling analysis (erDCA).
The gradient decent algorithm is implemented using c++ backend.

Authors:	Expectation Reflection Method: Evan Cresswell, Danh-Tai, Vipul Periwal 
			Python Module Frameowrk: Mehari B. Zerihun, Fabrizio Pucci
"""
...
class ErDCAException(Exception):
    """Implements exceptions related to ErDCA computation
    """

class ErDCA:
    """
	Python Module for Expectation Reflection
    """

    def __init__(self, msa_file, biomolecule, seqid = None, max_iterations = None, num_threads = None, 
            verbose = False):
        """Initilizes erdca instances
        """
        ...
        # erdcaBackend interface
		"""
        # extern "C" float* erdcaBackend(unsigned short const biomolecule, unsigned short const num_site_states, 
        ...
        self.__erdca = ctypes.CDLL(self.erdca_lib_path)
        self.__erdcaBackend = self.__erdca.erdcaBackend 
        self.__erdcaBackend.argtypes = (ctypes.c_ushort, ctypes.c_ushort, ctypes.c_char_p, ctypes.c_uint, 
        ...
        self.__erdcaBackend.restype = ctypes.POINTER(ctypes.c_float * self.__data_size)
        #extern "C" void freeFieldsAndCouplings(float*& fields_and_couplings)
        self.freeFieldsAndCouplings = self.__erdca.freeFieldsAndCouplings
        #self.freeFieldsAndCouplings.argtypes = (ctypes.POINTER(ctypes.c_float),) 
        self.freeFieldsAndCouplings.restype = None 
		"""
        log_message="""Created erDCA instance with:
            biomolecule: {}
            MSA sequence length: {}
    ...
    def biomolecule(self):
        """ErDCA biomolecule attribute 
			really just 'protein' for right noe -EC (1/2020)
        """
    ...
    def sequence_identity(self):
        """ErDCA sequence_identity attribute
        """
        return self.__seqid
    ...
    def max_iterations(self):
        """ErDCA max_iterations attribute
        """
    ...
    def effective_num_sequences(self):
        """ErDCA effective_num_sequences attribute
        """
    ...
    def get_fields_and_couplings_from_backend(self):
        """Compute fields and couplings using the C++ backend of erDCA
        ...
        for i, h_or_J in enumerate(h_J_ptr.contents):
            ...
            counter += 1
        #Free fields and couplings data from ErDCABackend
        ...
        try:
            ...
            logger.error('\n\tData size mismatch from the erDCA backend')
            raise
        else:
            logger.info('\n\tData size expected from erDCA backend: {}'.format(self.__data_size))
            logger.info('\n\tData size obtained from erDCA backend to Python: {}'.format(fields_and_couplings.size))
    ...
    def compute_sorted_FN_APC(self, seqbackmapper=None):
        ...
        couplings for the erDCA. If A SequenceBackmapper object is provided, 
        ...
        av_score_sites = list()
        N = self.__seqs_len
        scores_erdca = self.compute_sorted_FN()
        logger.info('\n\tPerforming average product correction (APC) of FN  of DCA scores')
        for i in range(N):
            i_scores = [score for pair, score in scores_erdca if i in pair]
        ...
        sorted_FN_APC = list()
        for pair, score in scores_erdca:
            i, j = pair
    ...
    def get_reg_single_site_freqs(self):
        ...
        # Currently DI scores for erDCA are computed using pseudocount = 0.5 
    ...
    def compute_direct_info_unsorted_DI(self):
        """Compute erDCA direct information score
