from pydca.er_dca import er_dca
...
def get_erdca_instance(msa_file, biomolecule, force_seq_type=False, **kwargs):
    """Inititalizes the ERDCA instance based on the arguments passed
    on the command line
    ...
    -------
        erdca_inst : ERDCA
            A ERDCA instance.
    """
    #erdca_inst = er_dca.ERDCA(msa_file, biomolecule)
...
def execute_from_command_line(msa_file=None, biomolecule=None, seqid=None,
    ...
    if the_command.strip() in DCA_COMPUTATION_SUBCOMMANDS:
        erdca_instance = get_erdca_instance(msa_file, biomolecule, seqid=seqid,
        ...
        if refseq_file:# do backmapping when reference sequence file is provided
            seqbackmapper = SequenceBackmapper(
                alignment_data=erdca_instance.alignment,
                refseq_file = refseq_file,
                biomolecule = erdca_instance.biomolecule
            )
        param_metadata = dca_utilities.erdca_param_metadata(erdca_instance)
        #create path to output directory is not supplied by user
        if not output_dir:
            msa_file_base_name, ext = os.path.splitext(os.path.basename(msa_file))
            output_dir = 'ERDCA_output_' + msa_file_base_name
        ...
        if the_command.strip()=='compute_di':
            if apc: # do average product correction if apc is passed from the command line
                sorted_DI = erdca_instance.compute_sorted_DI_APC(seqbackmapper=seqbackmapper)
                score_type = ' ER DI average product corrected (APC)'
                di_file_path = dca_utilities.get_dca_output_file_path(output_dir,
                    msa_file, prefix='ERDCA_apc_di_scores_', postfix='.txt'
                )
            else: # compute raw DCA score if apc is not asked
                sorted_DI = erdca_instance.compute_sorted_DI(seqbackmapper=seqbackmapper)
                score_type = 'raw DI'
                di_file_path = dca_utilities.get_dca_output_file_path(output_dir,
                    msa_file, prefix='ERDCA_raw_di_scores_', postfix='.txt'
                )
        ...
        if the_command.strip()=='compute_fn':
            if apc:
                score_type = 'ERDCA Frobenius norm, average product corrected (APC)'
                sorted_FN = erdca_instance.compute_sorted_FN_APC(seqbackmapper=seqbackmapper)
                fn_file_path = dca_utilities.get_dca_output_file_path(output_dir,
                    msa_file, prefix = 'ERDCA_apc_fn_scores_', postfix='.txt'
                )
            else:
                score_type = 'ERDCA raw Frobenius norm'
                sorted_FN = erdca_instance.compute_sorted_FN(seqbackmapper=seqbackmapper)
                fn_file_path = dca_utilities.get_dca_output_file_path(output_dir,
                    msa_file, prefix = 'ERDCA_raw_fn_scores_', postfix='.txt'
                )
        ...
        if the_command.strip()=='compute_fields':
            fields = erdca_instance.compute_fields()
            residue_repr_metadata = dca_utilities.erdca_residue_repr_metadata(
                erdca_instance.biomolecule
            )
        ...
        if the_command.strip() == 'compute_params':
            fields, couplings = erdca_instance.compute_params(
        ...
        if the_command.strip() == 'compute_fi':
            ...
            fi = erdca_instance.get_reg_single_site_freqs()
            residue_repr_metadata = dca_utilities.erdca_residue_repr_metadata(
                erdca_instance.biomolecule)
            metadata = param_metadata + residue_repr_metadata
            ...
            dca_utilities.write_single_site_freqs(fi_file_path, fi,
                seqs_len = erdca_instance.sequences_len,
                num_site_states = erdca_instance.num_site_states,
                metadata = metadata)
        ...
        if the_command.strip() == 'compute_fij':
            ...
            metadata = param_metadata + residue_repr_metadata
            fij = erdca_instance.get_reg_pair_site_freqs()
...
def run_er_dca():
    ...
    parser_compute_di = subparsers.add_parser('compute_di',
        ...
        ' Example: erdca compute_di <biomolecule> <MSA> --verbose, where'
